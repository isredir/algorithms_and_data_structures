...
void DFS_plus(int now, int from, Graph &graph, Graph &graph2) {
    graph.v[now] = true;
    for (int neig: graph.g[now]) {
        if (!graph.v[neig]) {
            DFS_plus(neig, now, graph, graph2);
            graph2.g[neig].push_back(now);
        } else if (neig != from) {
            graph2.g[now].push_back(neig);
            graph2.g[neig].push_back(now);
        }
    }
